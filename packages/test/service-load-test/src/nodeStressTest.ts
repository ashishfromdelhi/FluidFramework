...
async function main() {
    ...
    result = await orchestratorProcess(
        {loginInfo: { ...loginInfo, tenantFriendlyName: tenantArg },
         profile: { ...profile, name: profileArg },
         args: { url, numDoc, debug },
        });
    process.exit(result);
}
...
async function orchestratorProcess({ loginInfo, profile, args }:
    ...
    for (let j = 0; j < numDoc; j++) {
        ...
        const url = (_a = args.url) !== undefined && _a !== void 0 ? _a : await initialize(driveId, loginInfo);
        const estRunningTimeMin = Math.floor(2 * profile.totalSendCount /
            (profile.opRatePerMin * profile.numClients));
        console.log(`Connecting to ${args.url ? "existing" : "new"} 
        ...
        for (let i = 0; i < profile.numClients; i++) {
            const childArgs: string[] = [
                ...
                "--runId", (j * profile.numClients + i).toString(),
                "--url", url];
            if (args.debug) {
    ...
    const startDatetime_end = `Last Sync: ${  currentdate_end.getDate().toString()  }/${
    ...
    console.log(`Strat Time : ${startDatetime}`);
    console.log(`End Time : ${startDatetime_end}`);
